...
| **B-2** | **Tagged releases replace SHA pins** (L-9) — the capsule Dockerfile installs `…@2e62934…`. Standards require GitHub Releases and semantic versioning. | Capsule Dockerfile; forces a Code Ocean environment rebuild. | `v0.2.0` tag and GitHub Release exist (L-9 ✅). C-2 can now change the pin to `@v0.2.0`. |
...
Landed a MkDocs + mkdocstrings site via #26, but the RTD project was never imported so it never went live. Reverted in #29: docs hosting is deferred to SciComp's infrastructure rather than maintaining a second, unpublished hosting path. The `examples/` directory move (unrelated to hosting) was kept.
*Backing: page — "must be generated using MkDocs with mkdocstrings hosted through Read the Docs", root-level `docs/` and `examples/`. Size: M · Depends on: L-5 · Superseded by: #29*
...
| **L-10** (#13) · MkDocs + Read the Docs site | Landed (#26), then reverted (#29) — the RTD project was never imported, so docs hosting is deferred to SciComp's infrastructure instead of maintaining a second, unpublished path. The `examples/` directory move from the same PR was kept. |
...
Cutting L-8 and C-3 removed the plan's only serial multi-repo chain. Nothing now blocks on a breaking change except C-2, which waits on a tag. L-10 (docstrings → MkDocs API reference) is gone with the revert in #29.
